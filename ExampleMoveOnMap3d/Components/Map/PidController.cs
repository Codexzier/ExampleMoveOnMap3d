...
namespace ExampleMoveOnMap3d.Components.Map
{
    ...
    public class PidController
    {
        ...
        {
            this.Derivative = GainDerivative;
            this.Integral = GainIntegral;
            this.Proportional = GainProportional;
        ...
        public double ControlVariable(TimeSpan timeSinceLastUpdate)
        {
            ...
            IntegralTerm += (Integral * error * timeSinceLastUpdate.TotalSeconds);
            IntegralTerm = Clamp(IntegralTerm);

            // derivative term calculation
            double dInput = processVariable - ProcessVariableLast;
            double derivativeTerm = Derivative * (dInput / timeSinceLastUpdate.TotalSeconds);

            // proportional term calcullation
            double proportionalTerm = Proportional * error;
